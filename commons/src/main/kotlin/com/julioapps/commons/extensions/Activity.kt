...
import com.julioapps.commons.helpers.*
import com.julioapps.commons.models.Release
import com.julioapps.commons.models.SharedTheme
import com.julioapps.commons.views.MyTextView
...
fun Activity.appLaunched() {
    ...
    if (!isThankYouInstalled() && (baseConfig.appRunCount % 50 == 0)) {
        com.julioapps.commons.dialogs.DonateDialog(this)
    }
}
...
fun Activity.isShowingSAFDialog(file: File, treeUri: String, requestCode: Int): Boolean {
    return if (needsStupidWritePermissions(file.absolutePath) && (treeUri.isEmpty() || !hasProperStoredTreeUri())) {
        runOnUiThread {
            com.julioapps.commons.dialogs.WritePermissionDialog(this) {
...
fun com.julioapps.commons.activities.BaseSimpleActivity.checkWhatsNew(releases: List<Release>, currVersion: Int) {
    ...
    if (newReleases.isNotEmpty())
        com.julioapps.commons.dialogs.WhatsNewDialog(this, newReleases)

    baseConfig.lastVersion = currVersion
}

fun com.julioapps.commons.activities.BaseSimpleActivity.deleteFolders(folders: ArrayList<File>, deleteMediaOnly: Boolean = true, callback: ((wasSuccess: Boolean) -> Unit)? = null) {
...
fun com.julioapps.commons.activities.BaseSimpleActivity.deleteFoldersBg(folders: ArrayList<File>, deleteMediaOnly: Boolean = true, callback: ((wasSuccess: Boolean) -> Unit)? = null) {
...
fun com.julioapps.commons.activities.BaseSimpleActivity.deleteFolder(folder: File, deleteMediaOnly: Boolean = true, callback: ((wasSuccess: Boolean) -> Unit)? = null) {
    if (Looper.myLooper() == Looper.getMainLooper()) {
        Thread {
...
fun com.julioapps.commons.activities.BaseSimpleActivity.deleteFolderBg(folder: File, deleteMediaOnly: Boolean = true, callback: ((wasSuccess: Boolean) -> Unit)? = null) {
    if (folder.exists()) {
...
fun com.julioapps.commons.activities.BaseSimpleActivity.deleteFiles(files: ArrayList<File>, allowDeleteFolder: Boolean = false, callback: ((wasSuccess: Boolean) -> Unit)? = null) {
    if (Looper.myLooper() == Looper.getMainLooper()) {
        Thread {
...
fun com.julioapps.commons.activities.BaseSimpleActivity.deleteFilesBg(files: ArrayList<File>, allowDeleteFolder: Boolean = false, callback: ((wasSuccess: Boolean) -> Unit)? = null) {
    if (files.isEmpty()) {
        callback?.invoke(true)
        return
    }
...
fun com.julioapps.commons.activities.BaseSimpleActivity.deleteFile(file: File, allowDeleteFolder: Boolean = false, callback: ((wasSuccess: Boolean) -> Unit)? = null) {
    if (Looper.myLooper() == Looper.getMainLooper()) {
        Thread {
...
@SuppressLint("NewApi")
fun com.julioapps.commons.activities.BaseSimpleActivity.deleteFileBg(file: File, allowDeleteFolder: Boolean = false, callback: ((wasSuccess: Boolean) -> Unit)? = null) {
...
fun com.julioapps.commons.activities.BaseSimpleActivity.rescanDeletedFile(file: File, callback: (() -> Unit)? = null) {
...
@SuppressLint("NewApi")
fun com.julioapps.commons.activities.BaseSimpleActivity.renameFile(oldFile: File, newFile: File, callback: ((success: Boolean) -> Unit)? = null) {
...
fun com.julioapps.commons.activities.BaseSimpleActivity.getFileOutputStream(file: File, callback: (outputStream: OutputStream?) -> Unit) {
...
fun com.julioapps.commons.activities.BaseSimpleActivity.getFileOutputStreamSync(targetPath: String, mimeType: String, parentDocumentFile: DocumentFile? = null): OutputStream? {
...
@SuppressLint("NewApi")
fun com.julioapps.commons.activities.BaseSimpleActivity.getFileDocument(path: String): DocumentFile? {
    if (!isLollipopPlus())
        return null
...
fun Activity.handleHiddenFolderPasswordProtection(callback: () -> Unit) {
    if (baseConfig.isPasswordProtectionOn) {
        com.julioapps.commons.dialogs.SecurityDialog(this, baseConfig.passwordHash, baseConfig.protectionType) { hash, type, success ->
            if (success) {
                callback()
            }
        }
...
fun Activity.handleAppPasswordProtection(callback: (success: Boolean) -> Unit) {
    if (baseConfig.appPasswordProtectionOn) {
        com.julioapps.commons.dialogs.SecurityDialog(this, baseConfig.appPasswordHash, baseConfig.appProtectionType) { hash, type, success ->
            callback(success)
        }
    } else {
        callback(true)
...
fun com.julioapps.commons.activities.BaseSimpleActivity.createDirectorySync(directory: File): Boolean {
    if (directory.exists())
        return true
...
fun com.julioapps.commons.activities.BaseSimpleActivity.useEnglishToggled() {
...
fun com.julioapps.commons.activities.BaseSimpleActivity.restartActivity() {
    finish()
    startActivity(intent)
}
...
fun Activity.copyToClipboard(text: String) {
    val clip = ClipData.newPlainText(getString(R.string.julio_commons), text)
